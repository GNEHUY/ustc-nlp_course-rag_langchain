...
from database.create_vectordb import create_vectordb_info
...
EMBEDDING_MODEL_LIST = ['m3e', 'bge']
INIT_EMBEDDING_MODEL = "bge"
DEFAULT_DB_PATH = "/data/DeHors_yh_0329/code/RAG/RAG_Qa_sys_langchain/database/knowledge_db"
DEFAULT_PERSIST_PATH = "/data/DeHors_yh_0329/code/RAG/RAG_Qa_sys_langchain/database/vector_db/chroma"
...
class Model_center():
    ...
    def chat_qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "openai", embedding: str = "openai", temperature: float = 0.0, top_k: int = 5, history_len: int = 9, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        """
        调用带历史记录的问答链进行回答
        """
    ...
    def qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "openai", embedding="openai", temperature: float = 0.0, top_k: int = 5, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        """
        调用不带历史记录的问答链进行回答
        """
        if question == None or len(question) < 1:
...
with block as demo:
    with gr.Row(equal_height=True):           
        with gr.Column(scale=2):
            gr.Markdown("""<h1><center>基于检索增强的大模型回答系统（RAG）</center></h1>
                <center>喻恒，USTC，自然语言处理</center>
                """)
    ...
    with gr.Row():
        with gr.Column(scale=4):
            ...
            with gr.Row():
                # 创建提交按钮。
                db_with_his_btn = gr.Button("含历史信息的知识库RAG问答")
                db_wo_his_btn = gr.Button("不含历史信息的知识库RAG问答")
                llm_btn = gr.Button("使用语言模型问答")
            with gr.Row():
        ...
        init_db.click(create_vectordb_info,
    ...
    gr.Markdown("""提醒：<br>
    1. 项目自带数据库还包含Github的179K star的中文开源项目CS-Notes的md文件，包含基础知识、Leetcode、计算机操作系统、计算机网络、系统设计。
    2. 在使用之前，请先上传您自己的知识文件，否则系统将会解析项目自带的知识库。
    3. 数据库初始化可能需要一些时间，请耐心等候。 <br>
    致谢：感谢datawhale开源项目的教程指导""")
