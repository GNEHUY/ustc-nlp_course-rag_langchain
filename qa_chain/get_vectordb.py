...
import os
from database.create_vectordb import create_vectordb,load_knowledge_db
...
def get_vectordb(file_path:str=None, persist_path:str=None, embedding = "openai",embedding_key:str=None):
    ...
    """
    embedding = get_embedding(embedding_name=embedding, embedding_key=embedding_key)
    if os.path.exists(persist_path):  #持久化目录存在
        contents = os.listdir(persist_path)
        if len(contents) == 0:  #但是下面为空
            #print("目录为空")
            vectordb = create_vectordb(file_path, persist_path, embedding)
