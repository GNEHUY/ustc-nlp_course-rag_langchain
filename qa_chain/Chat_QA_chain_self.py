...
class Chat_QA_chain_self:
    ...
    - appid：星火
    - api_key：星火、OpenAI、智谱都需要传递的参数
    - Spark_api_secret：星火秘钥
    - embeddings：使用的embedding模型
    - embedding_key：使用的embedding模型的秘钥（智谱或者OpenAI）  
    """
    def __init__(self,model:str, temperature:float=0.0, top_k:int=5, chat_history:list=[], file_path:str=None, persist_path:str=None, appid:str=None, api_key:str=None, Spark_api_secret:str=None, embedding = "openai",embedding_key:str=None):
        self.model = model
    ...
    def answer(self, question:str=None,temperature = None, top_k = 5):
        ...
        qa = ConversationalRetrievalChain.from_llm(
                llm=llm,
                retriever=retriever,
                memory=self.memory,  # 显式绑定memory
                return_source_documents=True,
                get_chat_history=lambda h: h  # 添加历史记录格式转换
            )
